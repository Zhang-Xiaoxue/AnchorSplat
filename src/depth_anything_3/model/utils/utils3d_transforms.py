...
def generate_cameras(center: torch.Tensor, radius: float = .5, num_views: int = 300,
                     fov_xy: tuple[float, float] = (120, 80)) -> tuple[torch.Tensor, torch.Tensor]:
    '''
    Args:
        center (Tensor (B, 3)):
        radius (float):
        ...
        num_views (int):
    Returns:
        (extrinsics, intrinsics) (Tensor (B, V, 4, 4), (B, V, 3, 3)):
    '''
    yaw = torch.linspace(0, 2 * torch.pi, num_views, device=center.device)
    pitch = .1 * yaw.cos()
    sphere_offset = radius * torch.stack((
        yaw.sin() * pitch.cos(),
        yaw.cos() * pitch.cos(),
        pitch.sin()
    ), 1)
    center = center[:, None]
    origin = center + sphere_offset
    extrinsics = extrinsics_look_at(origin, center, center.new_tensor((0, 1, 0)))
    fov_xy = center.new_tensor(fov_xy).deg2rad()
    intrinsics = intrinsics_from_fov(fov_x=fov_xy[0], fov_y=fov_xy[1])
    return extrinsics, intrinsics.expand(*extrinsics.shape[:2], 3, 3)