...
def interpolate_camera(intrinsics: torch.Tensor, extrinsics: torch.Tensor,
                       num_views: int) -> tuple[torch.Tensor, torch.Tensor]:
    intrinsics, extrinsics = intrinsics.float(), extrinsics.float()
    interval = (num_views - 1) // (intrinsics.shape[1] - 1)
    t = torch.linspace(0, 1, interval + 2, device=intrinsics.device)[1:-1]
    new_intr = interpolate_intrinsics(intrinsics[:, :-1], intrinsics[:, 1:], t)
    new_extr = interpolate_extrinsics(extrinsics[:, :-1], extrinsics[:, 1:], t)
    new_intr = torch.cat((intrinsics[:, :-1, None], new_intr), 2).flatten(1, 2)
    new_extr = torch.cat((extrinsics[:, :-1, None], new_extr), 2).flatten(1, 2)
    return new_intr[:, :num_views], new_extr[:, :num_views]
